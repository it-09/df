...
try {
    ...
    if (itemsToPush.length > 0) {
        ...
        console.log(`Persisting ${itemsToPush.length} items...`);

        for (let i = 0; i < itemsToPush.length; i++) {
            try {
                const item = JSON.parse(JSON.stringify(itemsToPush[i]));

                await Actor.pushData(item);

                console.log(`✅ pushed item ${i + 1}`);
            } catch (err) {
                console.error(`❌ failed item ${i + 1}`, err);

                console.log(
                    JSON.stringify(itemsToPush[i], null, 2).slice(0, 5000)
                );
            }
        }

        console.log('Signal persistence finished');
    ...
    console.log(`Persisting ${aggregatedItems.length} items...`);

    for (let i = 0; i < aggregatedItems.length; i++) {
        try {
            const item = JSON.parse(JSON.stringify(aggregatedItems[i]));
            ...
            console.log(
                JSON.stringify(aggregatedItems[i], null, 2).slice(0, 5000)
            );
        }
    }
