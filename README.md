...
- **`OPENAI_API_KEY` Environment Variable**: The engine uses OpenAI (via OpenRouter) for the Stage 4 LLM Truth Layer. 
  > [!IMPORTANT]
  > **Bring Your Own Key (BYOK):** The person running this Actor must provide their own OpenAI/OpenRouter API key in the Apify Actor Environment Variables securely. You pay for the LLM token usage directly to your provider. 
  > 
  > **Graceful Fallback:** If you do not provide an API key, the engine will not break. It will automatically fall back to the Stage 1 & 2 heuristics. You will still get leads and intent scoring, but you will miss out on the deep CRM-ready reasoning and false-positive rejection.
...
└───────────────────────────┬─────────────────────────────┘
      ...
      [DISCARD / NOISE]    ┌─────────────────────────────┐
                           ...
                           │ • Human Buyer Validation    │
                           │ • False Positive Rejection  │
                           └─────────────┬───────────────┘
                                         │
                                         ▼
                           ┌─────────────────────────────┐
                           │ STAGE 4: CRM INTELLIGENCE   │
                           │ • Extract Pain Points       │
                           │ • Detect Switching Intent   │
                           │ • Draft SDR Outreach Angle  │
                           └─────────────┬───────────────┘
                                         │
                                         ▼
