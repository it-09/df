...
- **LinkedIn**: Captures professional intent, frustration with current tools, and evaluation requests (via Yahoo Dorking).
...
- **`OPENAI_API_KEY` Environment Variable**: The engine uses OpenAI (via OpenRouter) for the Stage 4 LLM Truth Layer. You must provide a valid API key in the Apify Actor Environment Variables securely. *Without it, the engine falls back to basic heuristics.*

### Source Toggles
- **`enableLinkedIn`**: Scrape professional B2B posts on LinkedIn (Recommended).
...
- **Zero Scraping API Costs**: Thanks to our Yahoo Search Dorking architecture, you do not need to pay for Google CSE, Proxies, or external scraping APIs.
